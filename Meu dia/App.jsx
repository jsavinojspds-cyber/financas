...
const Card = ({ children, style = {} }) => (
  <div style={{
    ...
    borderRadius: 14, overflow: "hidden", marginBottom: 12, ...style
  }}>{children}</div>
);
  ...
  <div style={{
    ...
    {total !== undefined && (
      <span style={{
        ...
        background: "rgba(255,255,255,.6)", borderRadius: 99, padding: "2px 8px"
      }}>{count}/{total}</span>
    )}
  </div>
);
...
const AlertBanner = ({ icon, title, sub, color, bg, border }) => (
  <div style={{
    background: bg, border: `1px solid ${border}`, borderRadius: 12,
    padding: "11px 14px", display: "flex", alignItems: "center", gap: 10, marginBottom: 8
  }}>
...
const EvItem = ({ e, nowMin, onToggle, color }) => {
  ...
  return (
    <div style={{
      ...
      background: isNow ? C.NAVYL : "transparent",
      borderLeft: isNow ? `3px solid ${C.NAVY}` : e.pers ? `3px solid ${C.AMBER}` : "3px solid transparent",
      opacity: e.done ? .35 : 1,
    }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {isNow && (
          <div style={{
            ...
            color: C.NAVY, background: C.NAVYL, border: `1px solid ${C.NAVYB}`,
...
const PdItem = ({ item, onToggle, dot }) => (
  <div style={{
    display: "flex", alignItems: "flex-start", gap: 11,
    padding: "11px 14px", borderBottom: `1px solid ${C.DIVIDER}`,
    opacity: item.done ? .35 : 1,
    background: item.urgent && !item.done ? C.REDL : "transparent",
  }}>
    <div onClick={() => onToggle(item.id)} style={{
      width: 20, height: 20, borderRadius: 6,
      border: `2px solid ${item.done ? C.GREEN : item.urgent ? C.REDB : C.BORDER}`,
      display: "flex", alignItems: "center", justifyContent: "center",
      cursor: "pointer", background: item.done ? C.GREEN : "transparent",
    ...
    <div style={{ flex: 1 }}>
      <div style={{
        ...
        color: item.urgent && !item.done ? C.RED : C.TEXT,
        fontWeight: item.urgent && !item.done ? 600 : 400,
        textDecoration: item.done ? "line-through" : "none",
      }}>{item.text}</div>
      {item.sub && (
        <div style={{ fontSize: 11, marginTop: 2, color: item.urgent && !item.done ? C.RED : C.MUTED }}>
          {item.sub}
        </div>
      )}
    </div>
    <div style={{
      ...
      background: item.urgent && !item.done ? C.RED : dot,
  ...
  return (
    <div style={{
      ...
      <div onClick={() => onToggle(t.id)} style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        cursor: "pointer", background: t.done ? C.GREEN : "transparent", flexShrink: 0,
      }}>
      ...
      <span style={{
        ...
        borderRadius: 99, background: bg, color: fg, flexShrink: 0,
      }}>{lbl}</span>
    </div>
  );
};
...
function useClock() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      const n = new Date(); setHh(n.getHours()); setMm(n.getMinutes());
    }, 30000);
...
function useVoice({ tasks, events, diary, setDiary, setTasksProf, setTasksPers }) {
  ...
  const process = async () => {
    ...
    try {
      ...
      if (p.action === "add_task_prof" && p.taskText)
        setTasksProf(prev => [...prev, { id: Date.now(), text: p.taskText, p: p.priority || "m", done: false }]);
      else if (p.action === "add_task_pers" && p.taskText)
        setTasksPers(prev => [...prev, { id: Date.now(), text: p.taskText, p: p.priority || "m", done: false }]);
      else if (p.action === "complete_task" && p.taskIndex != null) {
        const t = tasks[p.taskIndex];
        if (t) {
          setTasksProf(prev => prev.map(x => x.id === t.id ? { ...x, done: true } : x));
          setTasksPers(prev => prev.map(x => x.id === t.id ? { ...x, done: true } : x));
        }
      } else if (p.action === "add_note" && p.noteText) {
        const nd = diary ? `${diary}\n${p.noteText}` : p.noteText;
        setDiary(nd);
        lss("jd_dy_" + tk(), nd);
      }

      setVAi(p.response || "Feito.");
      setVSt("Concluído.");
      setVTxt("");
    } catch (e) {
      setVSt("Erro ao processar: " + e.message);
    }
    setVProc(false);
  };
...
export default function App() {
  ...
  const sortT   = arr => [...arr].sort((a, b) => {
    if (a.done !== b.done) return a.done ? 1 : -1;
    return ({ h: 0, m: 1, l: 2 }[a.p]) - ({ h: 0, m: 1, l: 2 }[b.p]);
  });
  ...
  const addTkP = () => {
    if (!ntP.trim()) return;
    setTkP(p => [...p, { id: Date.now(), text: ntP.trim(), p: "m", done: false }]);
    setNtP("");
  };
  const addTkE = () => {
    if (!ntE.trim()) return;
    setTkE(p => [...p, { id: Date.now(), text: ntE.trim(), p: "m", done: false }]);
    setNtE("");
  };
  const saveDiary = () => {
    lss("jd_dy_" + tk(), diary);
  ...
  const voice = useVoice({
    tasks: allTks, events: allEvts, diary, setDiary, setTasksProf: setTkP, setTasksPers: setTkE
  });
  ...
  const addBtn = bg => ({
    fontSize: 16, fontWeight: 700, width: 40, borderRadius: 10,
    ...
    <div style={{ background: C.BG, minHeight: "100vh", maxWidth: 520, margin: "0 auto", paddingBottom: 32, fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: `1px solid ${C.BORDER}`, padding: "16px 18px 14px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 700, color: C.TEXT, letterSpacing: -.5, fontFamily: "Georgia, serif", lineHeight: 1 }}>
              Meu Dia
            </div>
            <div style={{ fontSize: 12, color: C.MUTED, marginTop: 3 }}>
              Jean Savino · {new Date().toLocaleDateString("pt-BR", { weekday: "short", day: "numeric", month: "short", year: "numeric" })} · Manaus
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 30, fontWeight: 300, color: C.NAVY, fontFamily: "monospace", letterSpacing: -1, lineHeight: 1 }}>
              {p2(hh)}:{p2(mm)}
            </div>
      ...
      <div style={{ padding: "14px 14px 0" }}>
        ...
        {(hasUrgentPers || daysToMCP <= 1) && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
            {hasUrgentPers && (
              <AlertBanner icon="⚠️" title="Conta de energia vencida"
                sub="Venceu 10/05 · Pagar hoje" color={C.RED} bg={C.REDL} border={C.REDB} />
            )}
            {daysToMCP <= 1 && (
              <AlertBanner icon="✈️"
                title={daysToMCP <= 0 ? "Macapá HOJE — passagem não comprada!" : "Macapá amanhã — passagem não comprada"}
                sub="Ida 12/05 · Volta 15/05 · Comprar AGORA"
                color={C.AMBER} bg={C.AMBERL} border={C.AMBERB} />
            )}
          </div>
        )}
        ...
        <Card style={{ marginBottom: 8 }}>
          {evProf.length > 0
            ...
            : <div style={{ padding: "16px 14px", textAlign: "center", fontSize: 13, color: C.MUTED }}>Nenhum compromisso profissional hoje</div>
          }
        </Card>
        ...
        <Card style={{ marginBottom: 12 }}>
          {evPers.length > 0
            ...
            : <div style={{ padding: "16px 14px", textAlign: "center", fontSize: 13, color: C.MUTED }}>Nenhum compromisso pessoal hoje</div>
          }
        </Card>
        ...
        {wx && (
          <div style={{ background: C.NAVY, borderRadius: 14, overflow: "hidden", marginBottom: 12 }}>
            <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontFamily: "monospace", fontSize: 32, fontWeight: 300, color: "#fff", lineHeight: 1 }}>
                  {Math.round(wx.temperature_2m)}°C
                </div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.6)", marginTop: 3 }}>
                  {wDesc(wx.weather_code)} · Manaus
                </div>
              </div>
              <div style={{ marginLeft: "auto", display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                {[
                  `Sensação ${Math.round(wx.apparent_temperature)}°C`,
                  `Umidade ${wx.relative_humidity_2m}%`,
                  `Vento ${Math.round(wx.wind_speed_10m)} km/h`
                ].map(t => (
        ...
        <SecHdr color={C.NAVY} bg={C.NAVYL} border={C.NAVYB} icon="💼"
          title="Pendências Profissionais" count={pdP.filter(p => p.done).length} total={pdP.length} />
        <Card style={{ marginBottom: 8 }}>
          {pdP.map(p => <PdItem key={p.id} item={p} onToggle={togPdP} dot={C.NAVY} />)}
        </Card>

        {/* ── PENDÊNCIAS PESSOAIS ── */}
        <SecHdr color={C.AMBER} bg={C.AMBERL} border={C.AMBERB} icon="👤"
          title="Pendências Pessoais" count={pdE.filter(p => p.done).length} total={pdE.length} />
        <Card style={{ marginBottom: 12 }}>
        ...
        <SecHdr color={C.NAVY} bg={C.NAVYL} border={C.NAVYB} icon="💼"
          title="Tarefas Profissionais" count={tkP.filter(t => t.done).length} total={tkP.length} />
        <Card style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, padding: "8px 14px 12px", borderTop: `1px solid ${C.DIVIDER}` }}>
            <input value={ntP} onChange={e => setNtP(e.target.value)}
              onKeyDown={e => e.key === "Enter" && addTkP()}
        ...
        {/* ── TAREFAS PESSOAIS ── */}
        <SecHdr color={C.AMBER} bg={C.AMBERL} border={C.AMBERB} icon="👤"
          title="Tarefas Pessoais" count={tkE.filter(t => t.done).length} total={tkE.length} />
        <Card style={{ marginBottom: 12 }}>
          {sortT(tkE).map(t => <TkItem key={t.id} t={t} onToggle={togTkE} />)}
          <div style={{ display: "flex", gap: 8, padding: "8px 14px 12px", borderTop: `1px solid ${C.DIVIDER}` }}>
            <input value={ntE} onChange={e => setNtE(e.target.value)}
              onKeyDown={e => e.key === "Enter" && addTkE()}
        ...
        <Card>
          ...
          <div style={{ padding: "12px 14px 14px" }}>
            <textarea
              ...
              style={{ width: "100%", fontSize: 14, lineHeight: 1.6, padding: "10px 12px", background: "#f8f6f2", border: `1px solid ${C.BORDER}`, borderRadius: 10, color: C.TEXT, fontFamily: "inherit", outline: "none", resize: "vertical", minHeight: 80 }}
            />
            <button onClick={saveDiary} style={{ marginTop: 8, width: "100%", padding: "12px", fontSize: 14, fontWeight: 600, borderRadius: 10, background: C.NAVY, color: "white", border: "none", cursor: "pointer" }}>
              Salvar resumo
            </button>
          </div>
        </Card>
        ...
        <Card style={{ marginBottom: 0 }}>
          <div style={{ padding: "11px 14px 9px", borderBottom: `1px solid ${C.DIVIDER}` }}>
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", color: C.MUTED }}>
              Entrada por voz
            </span>
          </div>
          <div style={{ padding: "13px 14px 15px" }}>
            ...
            {voice.vTxt && (
              <div style={{ fontSize: 13, fontStyle: "italic", background: "#f8f6f2", border: `1px solid ${C.BORDER}`, borderRadius: 9, padding: "9px 12px", marginBottom: 10 }}>
                "{voice.vTxt}"
              </div>
            )}
            {voice.vAi && (
              <div style={{ fontSize: 13, color: C.NAVY, background: C.NAVYL, border: `1px solid ${C.NAVYB}`, borderRadius: 9, padding: "9px 12px", marginBottom: 10 }}>
                {voice.vAi}
              </div>
            )}
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={voice.vRec ? voice.stop : voice.start}
                style={{ flex: 1, padding: "12px", fontSize: 14, fontWeight: 600, borderRadius: 10, background: voice.vRec ? C.REDL : C.NAVY, color: voice.vRec ? C.RED : "white", border: voice.vRec ? `2px solid ${C.REDB}` : "none", cursor: "pointer" }}
              >
                {voice.vRec ? "⏹ Parar" : "🎙 Falar"}
              </button>
              {voice.vTxt && !voice.vRec && !voice.vProc && (
                <button onClick={voice.process} style={{ flex: 1, padding: "12px", fontSize: 14, fontWeight: 600, borderRadius: 10, background: C.GREEN, color: "white", border: "none", cursor: "pointer" }}>
                  Processar
                </button>
              )}
              {voice.vProc && (
                <div style={{ flex: 1, padding: "12px", fontSize: 13, color: C.MUTED, textAlign: "center", background: "#f8f6f2", borderRadius: 10 }}>
                  Processando...
                </div>
              )}
            </div>
            <div style={{ marginTop: 10, fontSize: 11, color: C.FAINT, lineHeight: 1.6 }}>
              Exemplos: "Adicionar tarefa profissional: ligar para Nailson" · "Marcar tarefa 1 como feita" · "Anota no resumo: pedido Comepi fechado"
            </div>
          </div>
        </Card>
